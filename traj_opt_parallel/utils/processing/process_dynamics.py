...
def do_process(inbag: str, downsamp: int, center: bool, perturb: bool, zrot: bool,
    ...
    file_pfix = ''
    if os.getenv(dirs.EXP_KEY) is not None:
        file_pfix = '_' + os.getenv(dirs.EXP_KEY)
    csvfn = datestr + file_pfix + '_temp.csv'
    ...
    for i in range(NTOSS):
        ...
        too_long = ends[i] - starts[i] > N_MAX
        ...
        if too_short or too_long or bad_contact or forces_in_air:
...
def do_process_multi(num: int, downsample: int, center: bool, perturb: bool, zrot: bool, pullback: bool, use_python3: bool, plot: bool,
    ...
    while accept_n < num:
        ...
        toss_groups = [fold for fold in toss_groups if 'tosses' not in fold]
